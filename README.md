...
**12. Fairness weighting is structurally sound but capacity-bound — the bottleneck is density, not allocation.** We added a fairness mechanism (`-fairness-beta`) that scales each edge weight by a symmetric factor ½(α_i+α_j), where α_i grows for stations with low worst-case SINR (high interference exposure). Because α_i is a fixed, allocation-independent property of a station (it depends only on neighbor positions and the frozen channel), the weighted game remains an exact potential game — a unit test confirms that a player's cost change still equals the global potential change under β>0, so the convergence guarantee survives. **But the mechanism does not improve cell-edge rate**, and the reason is the most important finding here: at N=40 in a 400×400 m area, **31 of 40 stations have ≥K neighbors**, so by pigeonhole a collision is mathematically unavoidable regardless of weighting — and indeed **99% of cell-edge stations already occupy the least-loaded color available to them**. The worst-served users are not victims of a poor allocation the game could fix; they are in outage because the network is denser than K colors can orthogonalize. No reweighting of a K-coloring cost function can manufacture spectrum that isn't there. This is a genuine result about the problem, not a defect: cell-edge relief in this regime requires more resources (larger K, power control, or cell densification with its own frequency plan), not a cleverer objective — which is precisely the kind of boundary a physically-grounded model can expose and a "conflict-count" model cannot.

**13. Paired analysis: the comparison the design actually licenses.** All schemes are evaluated within the same run, on the same topology and the same frozen channel, so per-run *differences* are the statistically correct unit — topology luck, which dominates the variance, cancels out. The Monte Carlo summary therefore reports paired differences (mean, median, 95% CI, paired t-test, win counts) alongside the marginal table. This matters: independent confidence intervals on the marginals overlap heavily and suggest "indistinguishable", while the paired test resolves the ordering. Both the mean and the median difference are reported, because a mean difference driven by a handful of outlier runs tells a different story than a consistent per-run advantage — and with 60 runs the two comparisons closest to zero (vs. greedy, vs. DSATUR) can flip between marginal significance and non-significance across independent replications. Report the paired table at `-runs 200`, and read the median and win count next to the mean rather than the p-value alone.
...
| `-fairness-beta` | 0 | A4: fairness strength; 0 = off, β>0 makes collisions costlier for high-interference-exposure (cell-edge) stations via a symmetric ½(α_i+α_j) weight that preserves the potential-game structure |
...
- **Fairness weighting (A4, `-fairness-beta`, default 0).** Each edge weight is scaled by ½(α_i+α_j) with α_i = exp(β·(SINR_ref − worstCaseSINR_i)/10). α_i is allocation-independent (worst-case SINR assumes all neighbors co-channel, using the frozen channel), so the game stays an exact potential game and convergence is preserved (unit-tested). It is provided for completeness and to demonstrate finding 12; it does not improve cell-edge rate because that regime is capacity-bound, not allocation-bound.
